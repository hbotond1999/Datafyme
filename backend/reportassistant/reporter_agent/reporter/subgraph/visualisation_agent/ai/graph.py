...
from reporter_agent.reporter.subgraph.visualisation_agent.ai.nodes import decide_representation, decide_chart_type, \
    populate_chart_data, create_final_data, validate_chart_data
...
def create_graph():
    ...
    graph.add_edge("populate_chart_data", "validate_chart_data")
    graph.add_conditional_edges("validate_chart_data", lambda x: len(x["error_messages"]) > 0,
                                {True: "populate_chart_data", False: "create_final_data"})
